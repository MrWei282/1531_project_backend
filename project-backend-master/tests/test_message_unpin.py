...
def setup():
    users = []
    users.append(register_user('czt1@email.com', 'czt0128', 'zhitao', 'chen'))
    users.append(register_user('czt2@email.com', 'czt0128', 'linbo', 'chen'))
    users.append(register_user('czt3@email.com', 'czt0128', 'tianyu', 'chen'))
...
def test_not_channel_owner(setup):
    users, channel, _ = setup
    id = message_channel(users[0]['token'], channel['channel_id'], "Howdy")['message_id']
    _,status_code = message_unpin(users[1]['token'], id)
